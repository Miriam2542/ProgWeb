...
from .views import CategoriaConfirmDeleteView
...
urlpatterns = [
    ...
    path('adicionar/materia/', MidiaCreate.as_view(), name="adicionar-materia"),
    ...
    path("editar/materia/<int:pk>/", MidiaUpdate.as_view(), name="editar-materia"),

    path("excluir/categoria/<int:pk>/", CategoriaConfirmDeleteView.as_view(), name="excluir-categoria"),
    ...
    path("excluir/materia/<int:pk>/", MidiaDelete.as_view(), name="excluir-materia"),
    ...
    path("noticia/<int:pk>/", NoticiaDetailView.as_view(), name="detalhar-noticia"),
    # Rota para limpar comentários de uma notícia (apenas Admin / superuser)
    path('noticia/<int:pk>/limpar-comentarios/', limpar_comentarios, name='limpar-comentarios'),
    # (rota de tarefas removida — não há handler atualmente)
]
